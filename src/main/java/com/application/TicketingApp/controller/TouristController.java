...
public class TouristController 
{
	...
	public ResponseEntity<?> getTourist(@PathVariable("id") Integer id)
	{
		try
		{
			return new ResponseEntity<Tourist>(service.findById(id),HttpStatus.OK);
	...
	public ResponseEntity<List<Tourist>> getAllTrourists()
	{
		return new ResponseEntity<List<Tourist>>(service.findAll(),HttpStatus.OK);
	}
	@GetMapping("/getTouristsbypackage/{packageType}")
	public ResponseEntity<?> getTouristbyPackageType(@PathVariable("packageType") String packageType)
	{
		try
		{
			return new ResponseEntity<List<Tourist>>(service.findByPackageType(packageType),HttpStatus.OK);
		}catch(TouristNotfoundException e)
		{
			return new ResponseEntity<String>(e.getMessage(),HttpStatus.BAD_REQUEST);
		}
	...
	@GetMapping("/getTouristsBetweenBudget/{budget1}/{budget2}")
	public ResponseEntity<?> getTouristsBetweenBudget(@PathVariable("budget1") Double budget1,@PathVariable("budget2") Double budget2)
	{
		try
		{
			return new ResponseEntity<List<Tourist>>(service.findByBudgetBetween(budget1,budget2),HttpStatus.OK);
		}catch(TouristNotfoundException e)
		{
			return new ResponseEntity<String>(e.getMessage(),HttpStatus.BAD_REQUEST);
		}
